...
int ParseHttpReqMesg(TopHttpRequest * pstHttpClientReq, char *sHttpReqBuf)
{
    ...
    WriteLog(LOG_NORMAL, "method = %s", method);
...
void* HandleClientRequestSoft(char* requestMsg, int clientfd) {
	...
	if(llResult == -1) {
        WriteLog(LOG_DEBUG, "client recv failed!");
        WriteLog(LOG_DEBUG, "client recv failed!");	
	...
	// 生成公私钥
	if(memcmp(pstHttpClientReq->Method, "300011", 6) == 0) {
	...
	// 申请3des密钥
	else if(memcmp(pstHttpClientReq->Method, "300001", 6) == 0) {
...
void* HandleClientRequestHard(char* requestMsg, int clientfd) {
	...
	if(encryptfd <= 0) {
        WriteLog(LOG_DEBUG, "Connect Encryption Machine failed!");
		return NULL;
    }
...
void* callback_func(void *arg, int clientfd) {
	...
	while(1) {
        ...
        {
			WriteLog(LOG_DEBUG, "recv msg:%s", recv_buf);
		    //  软加密
