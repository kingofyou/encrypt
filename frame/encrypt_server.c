...
int socket_bind(int host_port)
{
	...
	while(1) {
	    ...
	    if(-1 == sockfd)
	    {
		    WriteLog(LOG_ERROR, "socket server error");
			sleep(1);
			continue;
	    }
	    // 设置套接字选项避免地址使用错误   
	    ...
	    if(-1 == res)
	    {
		    WriteLog(LOG_ERROR, "bind server error");
			close(sockfd);
			sleep(1);
			continue;
	    }
...
void handle_accpet(int epollfd,int sockfd)
{
	...
	while(1) {
        ...
        if (clientfd == -1) {
            WriteLog(LOG_ERROR, "accpet error:");
			break;
		}
        else
        {
			...
			if(-1 == iret) {
				WriteLog(LOG_ERROR, "设置[%d]非阻塞失败", clientfd);
				exit(0);
			}
            //添加一个客户描述符和事件
            iret = add_event(epollfd,clientfd,EPOLLIN|EPOLLET);
			if(-1 == iret) {
				WriteLog(LOG_ERROR, "epoll_ctl add [%d]失败", clientfd);
				exit(0);
			}
        }
	}
