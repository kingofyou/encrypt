...
int pack300002(char*reqMsg, unsigned char* SendEncryptMsg, unsigned char* RecvEncryptMsg, int iLen, char* flag, int encryptfd) {
    char msgHead[8+1]={};
	...
	if(!strcmp("2", encryptFlag)) {	
        sprintf(directiveLength, "%04X", strlen(msgHead) + 2
			                            ...
			                            + 4
									    + strlen(dataTmp));
        sprintf(SendEncryptMsg, "%s%s%s%s%s%s%s%s%04d%s",
		                          msgHead,
		                          "TG",
		                          encryptFlag,
		                          mode,
	...
	// 解密
        ...
        sprintf(directiveLength, "%04X", strlen(msgHead) + 2
			                            ...
			                            + 4
									    + strlen(data)/2);
        sprintf(SendEncryptMsg, "%s%s%s%s%s%s%s%s%04d",
		                          msgHead,
		                          "TG",
		                          encryptFlag,
		                          mode,
...
int exec300002(char *reqMsg, char* SendEncryptMsg, char* RecvEncryptMsg, int encryptfd) {
	...
	char flag[2] = {};
    int sendLen = pack300002(reqMsg, SendEncryptMsg, RecvEncryptMsg, 4096, flag, encryptfd);
	if(sendLen <= 0) {
		return errMsg(RecvEncryptMsg);
	}
