...
int pack300005(char*reqMsg, unsigned char* SendEncryptMsg, int iLen) {
	char msgHead[8+1] = {};
	char zmkKey[64] = {};
	char zpkZmkKey[64] = {};
	...
	tmpJson = cJSON_GetObjectItem(root,"zpkZmkKey");
	if(tmpJson)
	    strcpy(zpkZmkKey, tmpJson->valuestring);
	else {
        free(root);
		return -1;
	}
    ...
    sprintf(directiveLength, "%04X", strlen(msgHead) + 2 + strlen(zmkKey) + strlen(zpkZmkKey));
    sprintf(SendEncryptMsg, "%s%s%s%s", msgHead, "FA", zmkKey, zpkZmkKey);	
...
int exec300005(char *reqMsg, char* SendEncryptMsg, char* RecvEncryptMsg, int encryptfd) {
    ...
    int sendLen = pack300005(reqMsg, SendEncryptMsg, 4096);
	if(sendLen <= 0) {
         return errMsg(RecvEncryptMsg);
	}
	...
	if(!strncmp("00", retCode, 2)) {
		...
		cJSON_AddStringToObject(root, "zpkLmkKey", key);  
