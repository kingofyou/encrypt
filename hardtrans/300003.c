/***********************************************************************************************
MAIN FUNCTION : 将ZEK/ZAK从ZMK转为LMK加密
AUTHOR        : Niu Lindong
...
int pack300003(char*reqMsg, unsigned char* SendEncryptMsg, int iLen, char* flg) {
	char msgHead[8+1] = {};
	char flag[1+1] = {};
	char zmkKey[64] = {};
	char zkKey[64] = {};
	...
	if(tmpJson)
	    strcpy(flag, tmpJson->valuestring);
	else {
        free(root);
		return -1;
	}
    ...
    sprintf(SendEncryptMsg, "%s%s%s%s%s", msgHead, "FK", flag, zmkKey, zkKey);	
...
int exec300003(char *reqMsg, char* SendEncryptMsg, char* RecvEncryptMsg, int encryptfd) {
	...
	if(!strlen(reqMsg)) return 0;
	char flg[2] = {};
    int sendLen = pack300003(reqMsg, SendEncryptMsg, 4096, flg);
	if(sendLen <= 0) {
         return errMsg(RecvEncryptMsg);
	}
	...
	if(!strncmp("00", retCode, 2)) {
		...
		if(!strcmp("1" , flg)) 
		    cJSON_AddStringToObject(root, "zakLmkKey", key);  
        else 
            cJSON_AddStringToObject(root, "zekLmkKey", key);  
